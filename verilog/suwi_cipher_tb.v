...
// Verifies encryption and decryption using provided test vectors
...
module suwi_cipher__tb;
    ...
    initial begin
        ...
        $display("Verifying %d test vectors for encryption and decryption.", 6);
        ...
        for (i = 0; i < 6; i = i + 1) begin
            ...
            $display("Cycles for Encryption  = %d", cycles);
            ...
            // Decryption
            text_in = ct[i];  // Use expected CT for decryption
            decrypt = 1;
            start = 1;
            #CLK_PERIOD start = 0;
            ...
            wait(done == 1);
            #CLK_PERIOD;
            $display("Cycles for Decription = %d", cycles);
            recovered_pt = text_out;

            if (recovered_pt == pt[i]) begin
                $display("Decryption PASS: Recovered PT = %h, Expected PT = %h", recovered_pt, pt[i]);
            end else begin
                $display("Decryption FAIL: Recovered PT = %h, Expected PT = %h", recovered_pt, pt[i]);
                errors = errors + 1;
            end
